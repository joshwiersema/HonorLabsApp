...
def build_doctor_summary(customer: dict, orders: list[dict] | None = None) -> dict:
    ...
    # Check both approval keys for maximum compatibility
    b2b_approved = extract_meta(meta, "b2bking_account_approved", "")
    b2b_approval = extract_meta(meta, "b2bking_account_approval", "")
...
async def customer_stats():
    ...
    for c in all_customers:
        ...
        if is_doc and cid not in seen_doctor_ids:
            ...
            b2b_approved = extract_meta(meta, "b2bking_account_approved", "")
            b2b_approval = extract_meta(meta, "b2bking_account_approval", "")
            if b2b_approved != "yes" and b2b_approval != "approved":
                pending_doctors += 1
